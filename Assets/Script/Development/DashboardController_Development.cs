...
public class DashboardController_Development : MonoBehaviour
{
    ...
    private void Awake()
    {
        Wall = prefabManager.Wall;
        TableTop = prefabManager.TableTop;
        FloorSurface = prefabManager.FloorSurface;

        HeadLevelDisplay = prefabManager.HeadLevelDisplay;
        WaistLevelDisplay = prefabManager.WaistLevelDisplay;
        GroundDisplay = prefabManager.GroundDisplay;
        WallDisplay = prefabManager.WallDisplay;
        TableTopDisplay = prefabManager.TableTopDisplay;

        armLength = prefabManager.armLength;
        HumanWaist = prefabManager.waist;
        LeftHand = prefabManager.leftHand;
        RightHand = prefabManager.rightHand;
        Shoulder = prefabManager.SphereCenter;
    ...
    private void Update()
    {
        if (prefabManager.Reset)
        {
            prefabManager.Reset = false;
    ...
    private void InitialiseEnvironment()
    {
        ...
        Array.Clear(prefabManager.FG.movingOBJs, 0, prefabManager.FG.movingOBJs.Length);
        Array.Clear(prefabManager.FG.previousMovingPosition, 0, prefabManager.FG.previousMovingPosition.Length);
        prefabManager.FG.rightMoving = false;
        prefabManager.FG.leftMoving = false;
        prefabManager.FG.leftFootToeCollision.TouchedObjs.Clear();
        prefabManager.FG.rightFootToeCollision.TouchedObjs.Clear();

        foreach (Transform t in currentDetailedViews.Values.ToList())
            Destroy(t.gameObject);
        currentDetailedViews.Clear();

        OriginalLandmarkParent = prefabManager.GetCurrentLandmarkParent();
        OriginalDetailedViewParent = prefabManager.GetCurrentDetailedViewParent();

        Landmark = prefabManager.GetCurrentLandmarkFOR();
        DetailedView = prefabManager.GetCurrentDetailedViewFOR();
    ...
    private Transform GenerateDetailedView(Transform t)
    {
        if (detailedViewParentList.ContainsKey(t.name))
        {
            ...
            visOnDetailedView.GetComponent<VisInteractionController_Development>().enabled = false;
    ...
    private void PositionLandmarks(ReferenceFrames landmark, List<Transform> originalLandmarks)
    {
        if (landmark == ReferenceFrames.Floor) // landmarks on floor
        {
            foreach (Transform t in originalLandmarks)
            {
                ...
                newLandmark.GetComponent<VisInteractionController_Development>().enabled = true;
            ...
            GroundDisplay.GetComponent<ReferenceFrameController_Development>().InitialiseLandmarkPositions(currentLandmarks.Values.ToList(), landmark);
        }
        else if (landmark == ReferenceFrames.Body) // landmarks on waist level display
        {
            ...
            WaistLevelDisplay.GetComponent<ReferenceFrameController_Development>().InitialiseLandmarkPositions(currentLandmarks.Values.ToList(), landmark);
        }
        else if (landmark == ReferenceFrames.Shelves) // landmarks on shelves
        {
            ...
            TableTopDisplay.GetComponent<ReferenceFrameController_Development>().InitialiseLandmarkPositions(currentLandmarks.Values.ToList(), landmark);
        }
    ...
    private void RePositionLandmarks(ReferenceFrames rf)
    {
        if (rf == ReferenceFrames.Body)
            WaistLevelDisplay.GetComponent<ReferenceFrameController_Development>().InitialiseLandmarkPositions(currentLandmarks.Values.ToList(), ReferenceFrames.Body);
        if (rf == ReferenceFrames.Shelves)
            TableTopDisplay.GetComponent<ReferenceFrameController_Development>().InitialiseLandmarkPositions(currentLandmarks.Values.ToList(), ReferenceFrames.Shelves);
    }
